"""
Production-quality single-file reference implementation for the Leave Management
System prompt.

This executable Python module models the backend behavior required by the prompt:
role-aware authentication, leave balance enforcement, weekday-only day counting,
pending-overlap prevention, department-restricted manager actions, admin policy
management, user deactivation, CSV reporting, structured JSON errors, and simple
rate limiting.
...
The HTTP server intentionally uses only the Python standard library so the
benchmark is easy to execute in isolation. A MERN production submission should
translate these same rules into Express, Mongoose, React, and Tailwind.
"""
...
class PasswordHasher:
    """Small standard-library password hasher for this executable benchmark."""
...
class LeaveManagementService:
    ...
    def list_requests(
        ...
        if actor.role == "employee":
            ...
            rows = [
                row for row in rows
                if self.users[row.user_id].department_id == actor.department_id
            ]
        ...
        if clean_search:
            rows = [
                row for row in rows
                if clean_search in self.users[row.user_id].name.lower()
            ]
    ...
    def list_users(self, actor: User) -> dict[str, Any]:
        ...
        users = [user.public() for user in self.users.values() if user.role != "admin"]
    ...
    def update_user(self, actor: User, user_id: int, payload: dict[str, Any]) -> User:
        ...
        if "email" in payload:
            ...
            if any(item.email == email and item.id != user.id for item in self.users.values()):
                raise ApiError(409, "Email already exists", "EMAIL_EXISTS")
            user.email = email
    ...
    def _balance_for(self, user_id: int) -> dict[str, int]:
        ...
        for request in self.requests.values():
            if request.user_id != user_id or request.status != "approved":
                continue
            if request.start_date.year == current_year:
                used[request.leave_type] += request.days
        policy = self.policy.public()
        return {
            leave_type: max(policy[leave_type] - used[leave_type], 0)
...
def make_handler(service: LeaveManagementService) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def _dispatch(self, method: str) -> None:
            try:
                ...
                if method == "POST" and path == "/api/auth/login":
                    ...
                    self._send_json(
                        {"success": True, "user": user.public()},
                        cookie=self._auth_cookie(token),
                    )
                    return
                ...
                if method == "PUT" and path.startswith("/api/admin/users/"):
                    ...
                    user = service.update_user(actor, user_id, body)
                ...
                error = ApiError(500, "Internal server error", "INTERNAL_ERROR")
                self._send_json(error.payload(), status=500)
        ...
        def _auth_cookie(self, token: str) -> str:
            return (
                f"token={token}; HttpOnly; Secure; SameSite=Lax; "
...
def run_self_test() -> None:
    ...
    balance = service.get_balance(employee)["data"]["annual"]
    assert balance == 15
